...
//Sınıf genişleten şeyler
require("discordjs-activity"); //Epic Paket, Daha fazla ayrıntı için: https://www.npmjs.com/package/discordjs-activity
require("./EpicPlayer"); //idk neden yapıyorum ama yeni bir şeyler öğrenmek istiyorum o yüzden...
...
class DiscordMusicBot extends Client {
  constructor(props) {
    ...
    this.database = {
      //jsoning node_modules dizinine kaydedildi, DOCS: https://jsoning.js.org/
      guild: new Jsoning("guild.json"), //Sunucu Yapılandırması
    };
    ...
    if (this.botconfig.Token === "")
      return new TypeError(
        "botconfig.js doldurulmaz. Lütfen hiçbir şeyin boş olmadığından emin olun, aksi takdirde bot düzgün çalışmayacaktır.."
      );
    ...
    //idk nerede yapacağım bu yüzden burada yaptım ;-;
    ...
    this.Manager = new Manager({
      ...
      .on("nodeConnect", (node) =>
        this.log(`Lavalink: ${node.options.identifier} düğümü bağlandı`)
      )
      .on("nodeError", (node, error) =>
        this.log(
          `Lavalink: ${node.options.identifier}  düğümünde bir hata oluştu: ${error.message}`
        )
      )
      .on("trackStart", async (player, track) => {
        this.SongsPlayed++;
        let TrackStartedEmbed = new MessageEmbed()
          .setAuthor(`Şimdi çalıyor ♪`, this.botconfig.IconURL)
      ...
      .on("queueEnd", (player) => {
        let QueueEmbed = new MessageEmbed()
          .setAuthor("Sıra sona erdi", this.botconfig.IconURL)
          .setColor(this.botconfig.EmbedColor)
          .setTimestamp();
        ...
        files.forEach((file) => {
          ...
          if (!cmd.name || !cmd.description || !cmd.run)
            return this.log(
              "Komut yüklenemiyor: " +
                file.split(".")[0] +
                ", Sebep: Dosya Yoktu run/name/desciption"
            );
    ...
    fs.readdir(EventsDir, (err, files) => {
      if (err) this.log(err);
        ...
        files.forEach((file) => {
          ...
          this.logger.log("Etkinlik Yüklendi: " + file.split(".")[0]);
        });
    });
  }
  ...
  sendError(Channel, Error) {
    let embed = new MessageEmbed()
      ...
      .setFooter(
        "Bunun bir hata olduğunu düşünüyorsanız, lütfen destek sunucusunda bildirin.!"
      );
  ...
  build() {
    ...
    if(this.botconfig.ExpressServer){
      this.http.listen(process.env.PORT || this.botconfig.Port, () => this.log("Web Sunucusu başlatıldı"));
    }
  }
