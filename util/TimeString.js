...
/**
 * Bir zaman dizesini ayrıştırırken kullanılacak varsayılan seçenekler
 *
 * @type {Object}
 */
...
const UNIT_MAP = {
  ...
  s: ["s", "sec", "secs", "second", "seconds", "sn"],
  m: ["m", "min", "mins", "minute", "minutes", "dk"],
  h: ["h", "hr", "hrs", "hour", "hours", "saat"],
  d: ["d", "day", "days", "gün"],
  w: ["w", "week", "weeks", "hafta"],
  mth: ["mon", "mth", "mths", "month", "months", "ay"],
...
function parseTimestring(string, returnUnit, opts) {
  ...
  if (groups === null) {
    throw new Error(`[${string}] dizesi, zaman dizesi tarafından ayrıştırılamadı`);
  }
...
/**
 * Geçilen seçeneklere göre birim değerleri alın
 *
 * @param   {Object} opts
 * @returns {Object}
 */
...
function getUnitKey(unit) {
  ...
  throw new Error(`[${unit}] birimi zaman dizisi tarafından desteklenmiyor`);
}
