...
module.exports = {
    name: "sıradöngü",
    description: "Tüm kuyruğu döngüye al",
    usage: "",
    permissions: {
      channel: ["VIEW_CHANNEL", "SEND_MESSAGES", "EMBED_LINKS"],
      member: [],
    },
    aliases: ["lq", "repeatqueue", "rq", "sıradöngü", "sd"],
    /**
      *
    ...
    run: async (client, message, args, { GuildDB }) => {
      ...
      if (message.guild.me.voice.channel && message.member.voice.channel.id !== message.guild.me.voice.channel.id) return client.sendTime(message.channel, ":x: | **Bu komutu kullanabilmek için benimle aynı ses kanalında olmalısınız!**");
        ...
        if (player.queueRepeat) {
          ...
          client.sendTime(message.channel, `:repeat: Kuyruk Döngüsü \`engelli\``);
        } else {
          player.setQueueRepeat(true)
          client.sendTime(message.channel, `:repeat: Kuyruk Döngüsü \`etkin\``);
        }
    },
    SlashCommand: {
       /**
       ...
       */
        run: async (client, interaction, args, { GuildDB }) => {
          ...
          let awaitchannel = client.channels.cache.get(interaction.channel_id); /// thanks Reyansh for this idea ;-;
            ...
            if(player.queueRepeat){
                  player.setQueueRepeat(false)
                  client.sendTime(interaction, `:repeat: **Kuyruk Döngüsü** \`engelli\``);
              }else{
                  player.setQueueRepeat(true)
                  client.sendTime(interaction, `:repeat: **Kuyruk Döngüsü** \`etkin\``);
              }
