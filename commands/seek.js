...
module.exports = {
    name: "ilerlemek",
    description: "Şarkıda bir pozisyona ilerle",
    usage: "<zaman sn/dk/sa>",
    permissions: {
        channel: ["VIEW_CHANNEL", "SEND_MESSAGES", "EMBED_LINKS"],
        member: [],
    },
    aliases: ["forward", "ilerle", "ileri"],
    /**
     *
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!player.queue.current.isSeekable) return client.sendTime(message.channel, "❌ | **Bu şarkıyı arayamıyorum!**");
        let SeekTo = client.ParseHumanTime(args.join(" "));
        if (!SeekTo) return client.sendTime(message.channel, `**Kullanım - **\`${GuildDB.prefix}ilerlemek <number sn/dk/sa>\` \n**Örnek - **\`${GuildDB.prefix}ilerlemek 2dk 10sn\``);
...
/*
    SlashCommand: {
        options: [
            {
                name: "position",
                description: "Aramak istediğiniz zaman damgasını girin. Örnek - 2dk 10sn",
                value: "position",
                type: 3,
                required: true,
                //**
