...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Kullanım - **\`${GuildDB.prefix}arama [sorgu]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      return client.sendTime(
        message.channel,
        SearchString + "için eşleşme bulunamadı"
      );
    else {
      ...
      let Pages = songs.map((songz) => {
        ...
        let em = new MessageEmbed()
          .setAuthor("Arama Sonuçları " + SearchString, client.botconfig.IconURL)
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Cevap vermen çok uzun sürdü. Bir şey çalmak istiyorsanız komutu tekrar çalıştırın!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(message.channel, "Lütfen doğru şarkı ID numarasını gönderin");
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song) return client.sendTime(message.channel, "Belirtilen ID için şarkı bulunamadı");
  ...
  SlashCommand: {
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      let awaitchannel = client.channels.cache.get(interaction.channel_id); /// bu fikir için teşekkürler Reyansh ;-;
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "NO_MATCHES":
            if (!player.queue.current) player.destroy();
            return client.sendTime(interaction, ":x: | **hiçbir sonuç bulunamadı**");
          case "TRACK_LOADED":
            player.queue.add(TrackUtils.build(Searched.tracks[0], member.user));
            if (!player.playing && !player.paused && !player.queue.length)
              player.play();
            return client.sendTime(
              interaction, `**Sıraya eklendi:** \`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri}}\`.`
            );
          ...
          case "PLAYLIST_LOADED":
            ...
            if (
              ...
              player.play();
            return client.sendTime(
              interaction, `**Oynatma listesi sıraya eklendi**: \n**${Searched.playlist.name}** \n**${Searched.playlistInfo.length} şarkı sıraya eklendi**`
            );
        }
      } else {
        try {
          ...
          return client.sendTime(
            interaction, `:x: | **Arama sırasında bir hata oluştu:** ${err.message}`
          );
        }
        switch (res.loadType) {
          case "NO_MATCHES":
          ...
          case "TRACK_LOADED":
            ...
            if (!player.playing && !player.paused && !player.queue.length)
              player.play();
            return client.sendTime(
              interaction, `**Sıraya eklendi:** \`[${res.tracks[0].title}](${res.tracks[0].uri})\`.`
            );
          case "PLAYLIST_LOADED":
            ...
            if (
              ...
              player.play();
            return client.sendTime(
              interaction, `**Oynatma listesi sıraya eklendi**: \n**${res.playlist.name}** \n**${res.playlistInfo.length} şarkı sıraya eklendi**`
            );
          case "SEARCH_RESULT":
            let max = 10,
              collected,
            ...
            const resultss = new MessageEmbed()
              ...
              .setColor(client.botconfig.EmbedColor)
              .setAuthor(`${search} için arama sonuçları`, client.botconfig.IconURL);
