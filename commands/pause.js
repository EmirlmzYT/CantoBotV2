...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (player.paused) return client.sendTime(message.channel, "❌ | **Müzik zaten duraklatıldı!**");
        player.pause(true);
        let embed = new MessageEmbed().setAuthor(`duraklatıldı!`, client.botconfig.IconURL).setColor(client.botconfig.EmbedColor).setDescription(`Oynamaya devam etmek için \`${GuildDB.prefix}devam et\` !`);
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (player.paused) return client.sendTime(interaction, "Müzik zaten duraklatıldı!");
            player.pause(true);
