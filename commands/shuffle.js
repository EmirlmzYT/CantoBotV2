...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!player.queue || !player.queue.length || player.queue.length === 0) return client.sendTime(message.channel, "❌ | **Sırada karıştırmak için yeterli şarkı yok!**");
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (!player) return client.sendTime(interaction.channel, "❌ | **Şu anda hiçbir şey çalmıyor...**");
            if (!player.queue || !player.queue.length || player.queue.length === 0) return client.sendTime(interaction, "❌ | **Sırada karıştırmak için yeterli şarkı yok!**");
            player.queue.shuffle();
