...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (message.guild.me.voice.channel && message.member.voice.channel.id !== message.guild.me.voice.channel.id) return client.sendTime(message.channel, ":x: | **Bu komutu kullanabilmeniz için benimle aynı ses kanalında olmanız gerekmektedir.!**");
        player.stop();
        await message.react("✅");
    },
    SlashCommand: {
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (skipTo !== null && (isNaN(skipTo) || skipTo < 1 || skipTo > player.queue.length)) return client.sendTime(interaction, "❌ | **Atlamak için geçersiz sayı!**");
            player.stop(skipTo);
