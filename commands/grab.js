...
run: async (client, message, args, { GuildDB }) => {
  ...
  if (!player.playing) return client.sendTime(message.channel, "❌ | **Şu anda hiçbir şey çalmıyor...**");
  if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ | **Bir şeyi çalmak için bir ses kanalında olmalısınız!**");
  ...
  .addField(`▶ Oynat:`, `\`${GuildDB ? GuildDB.prefix : client.botconfig.DefaultPrefix
  }play ${player.queue.current.uri}\``)
  .addField(`🔎 Şuraya kaydedildi::`, `<#${message.channel.id}>`)
  .setFooter(`Tarafından talep edildi: ${player.queue.current.requester.tag}`, player.queue.current.requester.displayAvatarURL({
    dynamic: true
  }))
    ).catch(e=>{
    ...
    client.sendTime(message.channel, "✅ | **Dm'lerini kontrol et!**")
  },
  SlashCommand: {
/**
*
...
*/
  run: async (client, interaction, args, { GuildDB }) => {
    ...
    if (!player.playing) return client.sendTime(interaction, "❌ | **Şu anda hiçbir şey çalmıyor...**")
    ...
    let embed = new MessageEmbed()
      ...
      return client.sendTime(interaction, "**:x: DM'leriniz devre dışı bırakıldı**")
    }

    client.sendTime(interaction, "✅ | **Dm'lerini kontrol et!**")
  },
  },
};
