...
module.exports = {
  name: "sözler",
  description: "Aranan şarkının sözlerini gösterir",
  usage: "[Şarkı adı]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!lyrics) return client.sendTime(message.channel, `**-** \`${SongTitle}\` için şarkı sözü bulunamadı`);
    lyrics = lyrics.split("\n"); //spliting into lines
    let SplitedLyrics = _.chunk(lyrics, 50); //45 lines each page

    let Pages = SplitedLyrics.map((ly) => {
      let em = new MessageEmbed()
        .setAuthor(`${SongTitle} Şarkı sözleri`, client.botconfig.IconURL)
  ...
  SlashCommand: {
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!interaction.data.options && !player) return client.sendTime(interaction, "❌ | **Şu anda hiçbir şey çalmıyor...**");
      ...
      if (lyrics.length === 0)
        return client.sendTime(interaction, `**-** \`${SongTitle}\` için şarkı sözü bulunamadı`);
      lyrics = lyrics.split("\n"); //spliting into lines
      let SplitedLyrics = _.chunk(lyrics, 50); //45 lines each page
