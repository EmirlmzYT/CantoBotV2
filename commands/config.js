...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    let Config = new MessageEmbed()
      ...
      .addField("Ön Ek", GuildDB.prefix, true)
      .addField("DJ Rolü", GuildDB.DJ ? `<@&${GuildDB.DJ}>` : "Not Set", true)
      .setDescription(`
Neyi düzenlemek istersin?
...
`);
    ...
    let emoji = await ConfigMessage.awaitReactions(
      ...
      client.sendTime(
        message.channel, "❌ | **Cevap vermen çok uzun sürdü. Ayarları düzenlemek istiyorsanız komutu tekrar çalıştırın.!**"
      );
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(message.channel, "Ön eki ne olarak değiştirmek istiyorsunuz??");
      let prefix = await message.channel.awaitMessages(
        (msg) => msg.author.id === message.author.id,
        { max: 1, time: 30000, errors: ["time"] }
      );
      if (!prefix.first())
        return client.sendTime(message.channel, "Cevap vermen çok uzun sürdü.");
      prefix = prefix.first();
      ...
      client.sendTime(
        message.channel, `Sunucu öneki başarıyla kaydedildi \`${prefix}\``
      );
    } else {
      await client.sendTime(
        message.channel, "Lütfen `DJ'lerin` sahip olmasını istediğiniz rolü belirtin."
      );
      ...
      if (!role.mentions.roles.first())
        return client.sendTime(
          message.channel, "Lütfen sadece DJ'ler için istediğiniz rolü belirtin."
        );
      ...
      client.sendTime(
        message.channel, "DJ rolü başarıyla kaydedildi: <@&" + role.id + ">"
      );
    }
  },
  ...
  SlashCommand: {
    options: [
      {
        name: "prefix",
        description: "Botun önekini kontrol edin",
        type: 1,
        required: false,
        options: [
          {
            name: "symbol",
            description: "Botun önekini ayarla",
            type: 3,
            required: false,
          },
        ],
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (config === "prefix") {
        ...
        if (
          ...
          client.sendTime(interaction, `Ön ek şimdi \`${prefix}\` olarak ayarlandı`);
        } else {
          //has not prefix
          client.sendTime(interaction, `Bu sunucunun öneki \`${GuildDB.prefix}\``);
        }
        ...
        if (
          ...
          client.sendTime(
            interaction, `Bu sunucunun DJ rolü başarıyla değiştirildi ${role.name}`
          );
        } else {
          /**
          ...
          client.sendTime(interaction, `Bu sunucunun DJ rolü ${role.name}`);
        }
      }
    },
  },
