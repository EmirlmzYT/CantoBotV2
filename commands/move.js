...
module.exports = {
    name: "taşı",
    description: "Bir parçayı belirli bir konuma taşır.",
    usage: "",
    permissions: {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!args[0] || !args[1]) return client.sendTime(message.channel, "❌ | **Geçersiz argümanlar.**");
        ...
        if (dest < 1 || dest > player.queue.length - 1) {
			return client.sendTime(message.channel, "❌ | **Geçersiz parkur hedefi.**");
        }
		...
		client.sendTime(message.channel, "✅ | **" + track.title + "** konuma taşındı " + (dest + 1) + ".");
    },
      ...
      options: [
          {
              ...
              type: 4,
              required: true,
              description: "Hareket etmek için takip edin.",
          },   
          {
              name: "position",
              value: "track2",
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (!args[0].value || !args[1].value) return client.sendTime(interaction, "❌ | **Geçersiz parça numarası.**");
            ...
            if (dest < 1 || dest > player.queue.length - 1) {
                return client.sendTime(interaction, "❌ | **Geçersiz parkur hedefi.**");
            }
            ...
            player.queue.splice(trackNum, 1);
            player.queue.splice(dest, 0, track);
            client.sendTime(interaction, "✅ | **" + track.title + "** konuma taşındı " + (dest + 1) + ".");
        },
    },
};