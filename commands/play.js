...
module.exports = {
    name: "oynat",
    description: "Şarkılarınızı Oynatın",
    usage: "[şarkı]",
    permissions: {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!SearchString) return client.sendTime(message.channel, `**Kullanım - **\`${GuildDB.prefix}play [song]\``);
        let CheckNode = client.Manager.nodes.get(client.botconfig.Lavalink.id);
        let Searching = await message.channel.send(":mag_right: Bulunuyor...");
        if (!CheckNode || !CheckNode.connected) {
       return client.sendTime(message.channel,"❌ | **Lavalink düğümü bağlı değil**");
        }
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`Oynatma listesi sıraya eklendi`, message.author.displayAvatarURL());
                    SongAddedEmbed.addField("Şarkılar", `\`${Searched.tracks.length}\` Sıraya Eklendi`, false);
                    ...
                    SongAddedEmbed.addField("Yazar", Searched.tracks[0].info.author, true);
                    ...
                    return client.sendTime(message.channel, SearchString + "**- için eşleşme bulunamadı**");
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, "❌ | **Şu anda hiçbir şey çalmıyor...**");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, SearchString + "**- için eşleşme bulunamadı**");
                else if (Searched.loadType == "PLAYLIST_LOADED") {
                    player.queue.add(Searched.tracks);
                    if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
                    SongAddedEmbed.setAuthor(`Oynatma listesi sıraya eklendi`, client.botconfig.IconURL);
                    SongAddedEmbed.setThumbnail(Searched.tracks[0].displayThumbnail());
                    SongAddedEmbed.setDescription(`[${Searched.playlist.name}](${SearchString})`);
                    SongAddedEmbed.addField("Şarkı", `\`${Searched.tracks.length}\` sıraya alındı`, false);
                    ...
                    SongAddedEmbed.addField("Süre", `\`${prettyMilliseconds(Searched.tracks[0].duration, { colonNotation: true })}\``, true);
                    if (player.queue.totalSize > 1) SongAddedEmbed.addField("Sıradaki konumu", `${player.queue.size - 0}`, true);
                    Searching.edit(SongAddedEmbed);
                }
            }
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    case "LOAD_FAILED":
                        ...
                        return client.sendError(interaction, `❌ | **Arama sırasında bir hata oluştu**`);

                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                        return client.sendTime(interaction, "❌ | **Hiçbir sonuç bulunamadı.**");
                    case "TRACK_LOADED":
                    ...
                    case "SEARCH_RESULT":
                        ...
                        let SongAdded = new MessageEmbed();
                            ...
                            if (player.queue.totalSize > 1) SongAdded.addField("Sıradaki konumu", `${player.queue.size - 0}`, true);
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        Playlist.setAuthor(`Oynatma listesi sıraya eklendi`, client.botconfig.IconURL);
                        Playlist.setDescription(`[${Searched.playlistInfo.name}](${interaction.data.options[0].value})`);
                        Playlist.addField("Şarkı", `\`${Searched.tracks.length}\` sıraya eklendi`, false);
                ...
                switch (res.loadType) {
                    ...
                    case "TRACK_LOADED":
                        ...
                        let SongAddedEmbed = new MessageEmbed();
                            ...
                            SongAddedEmbed.addField("Süre", `\`${prettyMilliseconds(res.tracks[0].duration, { colonNotation: true })}\``, true);
                            if (player.queue.totalSize > 1) SongAddedEmbed.addField("Sıradaki konumu", `${player.queue.size - 0}`, true);
                            return interaction.send(SongAddedEmbed);
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        await player.play();
                        let SongAdded = new MessageEmbed();
                        SongAdded.setAuthor(`Oynatma listesi sıraya eklendi`, client.botconfig.IconURL);
                        ...
                        SongAdded.addField("Şarkı", `\`${res.tracks.length}\` sıraya eklendi`, false);
                        SongAdded.addField("Oynatma listesi süresi", `\`${prettyMilliseconds(res.playlist.duration, { colonNotation: true })}\``, false);
                        return interaction.send(SongAdded);
                    case "SEARCH_RESULT":
                        ...
                        if (!player.playing && !player.paused && !player.queue.length) {
                            ...
                            SongAddedEmbed.addField("Süre", `\`${prettyMilliseconds(track.duration, { colonNotation: true })}\``, true);
                            if (player.queue.totalSize > 1) SongAddedEmbed.addField("Sıradaki konumu", `${player.queue.size - 0}`, true);
                            player.play();
                            return interaction.send(SongAddedEmbed);
